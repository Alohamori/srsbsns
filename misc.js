...
const pass = [], deck = [...Array(names.length).keys()];
...
const judge = () => {
  ...
  qs('#current').innerHTML = `<span id="score">${score}</span><span id="outof">/ 75</span>`;
...
const handle = si => {
  ...
  const ti = Array.from(seated).find(e => names[e.getAttribute('data-i')] > names[ni]);
